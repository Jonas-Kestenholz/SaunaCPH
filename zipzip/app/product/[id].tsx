...
function getProductAction(params: {
  ...
  const { selectedVariant, hasAnySoldOutVariants, allVariantsAvailable } = params;
...
function closeSizeSheet(callback?: () => void) {
  ...
  const displayPrice =
    selectedVariant?.price ?? data?.variants[0]?.price ?? 'Pris ikke tilgængelig';
  ...
  >
    ...
    >
      ...
      >
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ marginTop: 4, fontSize: 13 }}>
            SIZE: {selectedVariant ? getVariantLabel(selectedVariant.title) : '-'}
          </Text>
          <Text style={{ marginTop: 4, fontSize: 13 }}>{displayPrice}</Text>
        </View>
      </View>
      ...
      >
        <Text style={{ color: '#fff', fontWeight: '800' }}>VIEW CART</Text>
      </Pressable>
    </Animated.View>
  </View>
) : null}
